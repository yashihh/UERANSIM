...
sudo iptables -t mangle -A PREROUTING -i enp3s0 -s 172.168.56.3 -j TEE --gateway 10.60.0.0
...
sudo ip netns exec TSN ip addr add 172.168.56.2/24 dev enp2s0
sudo ip netns exec TSN ip r add 10.60.0.0/24 dev uesimtun0
sudo ip netns exec TSN iptables -t mangle -A PREROUTING -i enp2s0 -s 172.168.56.10 -j TEE --gateway 10.60.0.0
